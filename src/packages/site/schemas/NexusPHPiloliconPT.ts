...
> = {
  ...
  userInfo: {
     ...
     * ilolicon PT不使用id而使用uuid
     * 部分 NPHP 站点允许修改 name，所以 name 不能视为不变 ！！！
     */
    pickLast: ["uuid", "joinTime"],
    selectors: {
      // "page": "/index.php",
      uuid: {
      ...
      // uuid: {
      //   selector: [
      //     "a[href*='userdetails.php'][class~='name']:first-child",
      //     "a[href*='userdetails.php'][class~='rainbow']:first-child"
      //   ],
      //   attr: "href",
      //   filters: [{ name: "querystring", args: ["uuid"] }],
      // },
      ...
      //   ],
      // },
      // "page": "/userdetails.php?uuid=$user.uuid$",
      messageCount: {
        text: 0,
...
export default class NexusPHPiloliconPT extends PrivateSite {
   ...
   * @protected
   */
  protected async requestUserSeedingPage(userUuid: string, type: string = "seeding"): Promise<string | null> {
    const { data } = await this.request<string>({
      url: "/getusertorrentlistajax.php",
      params: { useruuid: userUuid, type },
    });
    return data || null;
  }

  protected async parseUserInfoForSeedingStatus(flushUserInfo: Partial<IUserInfo>): Promise<Partial<IUserInfo>> {
    const userUuid = flushUserInfo.uuid as string;
    const userSeedingRequestString = await this.requestUserSeedingPage(userUuid);
  ...
  protected async parseUserInfoForUploads(flushUserInfo: Partial<IUserInfo>): Promise<Partial<IUserInfo>> {
    const userUuid = flushUserInfo.uuid as string;
    const userUploadsRequestString = await this.requestUserSeedingPage(userUuid, "uploaded");
